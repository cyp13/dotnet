...
<script>
									...
									function(data) {
										...
										if (a == 1) {//开户
											...
											if (statis == 0) {//渠道第一次提交 ---280及产品类型确认
												...
												//进入这个页面那么提交的状态就改变了
												$("#work_statis1").val("1"); 
												$("#editForm").attr("action","${ctx}/jsp/work/updateCommerce.do"); 
										...
										/* if (row.service_type_detail) { //李:业务小类相关得
											alert(1)
											$("#service_type").change();
											$("#service_type_detail").val(
													row.service_type_detail);
										} */
										/* if (row.files) {//李;不知道如何
											if (flag != '3') {
												getFiles(row.files, "#fileList");
											} else {
												getFiles(row.files,
	...
	function isorno(value) {
		...
		if (value == '驳回') {
			...
			$("#editForm").attr("action","${ctx}/jsp/work/subCommerce.do"); //如果是驳回就修改为提交url
	...
	function isorno1_3(value) {
		...
		if (value == '驳回') {
			...
			var a = $("#work_statis1").val();
			if(parseInt(a)==1||parseInt(a)==2){//个人是1 或2  不流转流程 只是修改数据
				$("#editForm").attr("action","${ctx}/jsp/work/subCommerce.do"); 
			}
		}
	}
	/*
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<form id="editForm" action="${ctx}/jsp/work/subCommerce.do"
							...
							<div class="dataForm">
								...
								<div id="work_kaihu" style="display: none;">
									<div class="form-group">
										...
										<div class="col-sm-4">
											<s:dict dictType="zxgd" name="work_type" type="select"
