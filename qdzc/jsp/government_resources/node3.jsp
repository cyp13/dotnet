 	...
 	$(window).ready(function() {
 		...
 		if(pid){
			...
			ajax(obj, function(data){
				var row = data.Rows[0];
				console.log(row);
				if (row.sjy_result != null) {
					row.sjy_result = "PON数据专线/数据申请";
				}
				if (row.sign != null) {
					row.sign = "1";
				}
				if (row.merge_sign != null) {
					row.merge_sign = "0";
				}
				$("#editForm").fill(row);
		...
		$("#btn-ok").on("click", function() {
		 ...
		 $('input[name=sjy_result]').change(function(){
			 if ($(this).val() == "PON数据专线/数据申请") {
				$('#opinion textarea').removeClass('validate[required]');
				$('#sgfa').show();
				$('#sgfa input').addClass('validate[required]');
				$('#sjsqb').show();
				$('#sjsqb input').addClass('validate[required]');
				$('#sign').val("1");
			}else if ($(this).val() == "非PON/数据申请") {
				$('#opinion textarea').removeClass('validate[required]');
				$('#sgfa').show();
				$('#sgfa input').addClass('validate[required]');
				$('#sjsqb').show();
				$('#sjsqb input').addClass('validate[required]');
				$('#sign').val("2");
			}else if ($(this).val() == "PON互联网无资源") {
				$('#opinion textarea').removeClass('validate[required]');
				$('#sgfa input').addClass('validate[required]');
				$('#sgfa').show();
				$('#sjsqb').hide();
				$('#sjsqb input').removeClass('validate[required]');
				$('#sign').val("4");
			}else if ($(this).val() == "回退到全业务支撑") {
				$('#opinion textarea').removeClass('validate[required]');
				$('#sgfa').hide();
				$('#sgfa input').removeClass('validate[required]');
				$('#sjsqb').hide();
				$('#sjsqb input').removeClass('validate[required]');
			}else {
		            ...
		            <div class="ibox-content">
                       <form id="editForm" action="${ctx}/jsp/work/subGovernmentResources.do"  method="post" class="form-horizontal key-13" >
						   ...
						   <div class="form-group">
								...
								<div class="col-sm-4">
									<label>
										<input type="radio" name="sjy_result" value="PON数据专线/数据申请" checked="checked" id="archive"/><span style="padding: 0px 10px 0px 5px;">PON数据专线/数据申请</span>
									</label>
									<label>
										<input type="radio" name="sjy_result" value="PON互联网无资源"/><span style="padding: 0px 10px 0px 5px;">PON互联网无资源</span>
									</label> 
									<label>
										<input type="radio" name="sjy_result" value="非PON/数据申请"/><span style="padding: 0px 10px 0px 5px;">非PON/数据申请</span>
									</label> 
									<label>
										<input type="radio" name="sjy_result" value="回退到装维"/><span style="padding: 0px 10px 0px 5px;">退回</span>
									</label> 
									<label>
										<input type="radio" name="sjy_result" value="回退到全业务支撑"/><span style="padding: 0px 10px 0px 5px;">回退到全业务支撑</span>
									</label> 
								</div>
							</div>
							<div class="form-group" id="sgfa">
							   	<label  class="col-sm-2 control-label">施工方案</label>
							    <div class="col-sm-9">
							    	<input type="file" class="input-sm form-control validate[required]" multiple name="file" id="file" >
							    </div>
						   </div>
						   <div class="form-group" id="sjsqb">
							   	<label  class="col-sm-2 control-label">数据申请表</label>
							    <div class="col-sm-9">
							    	<input type="file" class="input-sm form-control validate[required]" multiple name="file" id="file" >
							    </div>
						   </div>
