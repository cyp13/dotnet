...
var queryNodeNameUrl = "${ctx}/jsp/work/queryNodeNameForGovernmentResources.do"
...
var taskName = '${param.taskName}';
 	...
 	//查询可以退回的节点
 	function getNodeName(pid){
 		$("#back").html('');//清空人员信息
 		var param = new Object();
 		param.url=queryNodeNameUrl;
 		param.pid = pid;
 		ajax(param, function(data){
 			console.log(data.Rows);
 			if(data.Rows.length>0){
 				formatData1(data.Rows,"#back");
 			}else{
 				alert("当前无可退回的节点！");
 				return;
 			}
 	...
 	//用于将数据封装成下拉框等形式。
	function formatData1(data,element){
		var htm = '';
		$.each(data,function(index,o){
			htm+='<option value="'+ '退到' + o +'">'+ o +'</option>';
		});
		if($(element)){
			$(element).html(htm);
		            ...
		            <div class="ibox-content">
                       <form id="editForm" action="${ctx}/jsp/work/subGovernmentResources.do"  method="post" class="form-horizontal key-13" >
						   ...
						   <div class="form-group">
								...
								<div class="col-sm-4">
									<select type="text" name="result" id="back" class="form-control selectpicker validate[required]"></select>
								</div>
							</div>
