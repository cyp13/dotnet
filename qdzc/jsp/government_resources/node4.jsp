...
var taskName = '${param.taskName}';
 	$(window).ready(function() {
		...
		$("#btn-ok").on("click", function() {
		 ...
		 $('input[name=sgdw_result1]').change(function(){
			 if ($(this).val() == "完成施工（PON数据专线）") {
		 ...
		 $('input[name=sgdw_result2]').change(function(){
			 if ($(this).val() == "完成施工（非PON）") {
				 $('#opinion textarea').removeClass('validate[required]');
				 $('#merge_sign').val("y");
			}else if ($(this).val() == "回退到全业务支撑") {
				$('#opinion textarea').removeClass('validate[required]');
		...
		$('input[name=sgdw_result3]').change(function(){
			 if ($(this).val() == "回退") {
				 $('.accessory').hide();
		...
		$('input[name=sgdw_result4]').change(function(){
			 if ($(this).val() == "完成施工（PON互联网无资源）") {
				 $('.accessory').show();
				 $('.accessory input').addClass('validate[required]');
				 $('#opinion textarea').removeClass('validate[required]');
			}else if ($(this).val() == "回退到全业务支撑") {
		...
		$('input[name=sgdw_result5]').change(function(){
			 if ($(this).val() == "回退") {
				 $('.accessory').hide();
				 $('.accessory input').removeClass('validate[required]');
				 $('#opinion textarea').addClass('validate[required]');
                ...
                <div class="ibox float-e-margins">
		            ...
		            <div class="ibox-content">
                       <form id="editForm" action="${ctx}/jsp/work/subGovernmentResources.do"  method="post" class="form-horizontal key-13" >
						  ...
						  <!--  PON数据专线/数据申请 设计院处理后和退回的工单（阶段：施工-光缆） -->
						  <div id="handling1">
						  <input type="hidden" name="merge_sign" id="merge_sign" value="y">
							   <div class="form-group">
									<label  class="col-sm-2 control-label">处理方式</label>
									<div class="col-sm-4">
										<label>
											<input type="radio" name="sgdw_result1" value="完成施工（PON数据专线）" checked="checked"/><span style="padding: 0px 10px 0px 5px;">完成施工</span>
										</label>
										<label>
											<input type="radio" name="sgdw_result1" value="异常"/><span style="padding: 0px 10px 0px 5px;">异常</span>
										</label> 
										<label>
											<input type="radio" name="sgdw_result1" value="回退到设计院"/><span style="padding: 0px 10px 0px 5px;">退回</span>
										</label>
										<label>
											<input type="radio" name="sgdw_result1" value="回退到全业务支撑"/><span style="padding: 0px 10px 0px 5px;">回退到全业务支撑</span>
										</label> 
									</div>
								</div>
							...
							<!-- 非PON/数据申请 设计院处理后和退回的工单（阶段：施工-光缆） -->
						  <div id="handling2">
						  <input type="hidden" name="merge_sign" id="merge_sign">
							   <div class="form-group">
									<label  class="col-sm-2 control-label">处理方式</label>
									<div class="col-sm-4">
										<label>
											<input type="radio" name="sgdw_result2" value="完成施工（非PON）" checked="checked"/><span style="padding: 0px 10px 0px 5px;">完成施工</span>
										</label>
										<label>
											<input type="radio" name="sgdw_result2" value="异常"/><span style="padding: 0px 10px 0px 5px;">异常</span>
										</label> 
										<label>
											<input type="radio" name="sgdw_result2" value="回退到设计院"/><span style="padding: 0px 10px 0px 5px;">退回</span>
										</label>
										<label>
											<input type="radio" name="sgdw_result2" value="回退到全业务支撑"/><span style="padding: 0px 10px 0px 5px;">回退到全业务支撑</span>
										</label> 
									</div>
								</div>
								<div class="form-group" id="opinion">
							...
							<!-- PON数据专线/数据申请 设计院处理后和退回的工单（阶段：施工-光缆）——> 选择“完成施工”点击“确定”流转到自己 -->
							<div id="handling3">
							<input type="hidden" name="merge_sign" id="merge_sign" value="0">
							<input type="hidden" name="sign" id="sign" value="0">
								<div class="form-group">
								<label  class="col-sm-2 control-label">处理方式</label>
									<div class="col-sm-4">
										<label>
											<input type="radio" name="sgdw_result3" value="挂测" checked="checked"/><span style="padding: 0px 10px 0px 5px;">挂测</span>
										</label>
										<label>
											<input type="radio" name="sgdw_result3" value="回退"/><span style="padding: 0px 10px 0px 5px;">退回</span>
										</label> 
										<label>
											<input type="radio" name="sgdw_result3" value="回退到全业务支撑"/><span style="padding: 0px 10px 0px 5px;">回退到全业务支撑</span>
										</label> 
									</div>
								</div>
							...
							<!-- PON互联网无资源 设计院处理后和退回的工单（阶段：施工-光缆） -->
							<div id="handling4">
								<div class="form-group">
								<label  class="col-sm-2 control-label">处理方式</label>
									<div class="col-sm-4">
										<label>
											<input type="radio" name="sgdw_result4" value="完成施工（PON互联网无资源）" checked="checked"/><span style="padding: 0px 10px 0px 5px;">完成施工</span>
										</label>
										<label>
											<input type="radio" name="sgdw_result4" value="异常"/><span style="padding: 0px 10px 0px 5px;">异常</span>
										</label> 
										<label>
											<input type="radio" name="sgdw_result4" value="回退"/><span style="padding: 0px 10px 0px 5px;">退回</span>
										</label> 
										<label>
											<input type="radio" name="sgdw_result4" value="回退到全业务支撑"/><span style="padding: 0px 10px 0px 5px;">回退到全业务支撑</span>
										</label> 
									</div>
								</div>
								<div class="form-group accessory">
							...
							<!-- 非PON/数据申请 设计院处理后和退回的工单（阶段：施工-光缆）——> 选择“完成施工”点击“确定”流转到自己 -->
							<div id="handling5">
							<input type="hidden" name="merge_sign" id="merge_sign" value="0">
							<input type="hidden" name="sign" id="sign" value="0">
								<div class="form-group">
								...
								<div class="col-sm-4">
									<label>
										<input type="radio" name="sgdw_result5" value="完成业务开通" checked="checked"/><span style="padding: 0px 10px 0px 5px;">完成业务开通</span>
									</label>
									<label>
										<input type="radio" name="sgdw_result5" value="回退"/><span style="padding: 0px 10px 0px 5px;">退回</span>
									</label> 
									<label>
										<input type="radio" name="sgdw_result5" value="回退到全业务支撑"/><span style="padding: 0px 10px 0px 5px;">回退到全业务支撑</span>
									</label> 
								</div>
							</div>
							<div class="form-group accessory">
