...
var taskName = '${param.taskName}';
 	$(window).ready(function() {
		...
		$("#btn-ok").on("click", function() {
			...
			if(b) {
				...
				ajaxSubmit("#editForm",pa, function(data) {
					if ("error" == data.resCode) {
						...
						if (data.resMsg == "没有可以回退的路径！") {
							layer.msg("已经是第一步发起人，不可以再进行会退了！", {icon: 2});
						}else {
		 ...
		 $('input[name=zdzc_result]').change(function(){
			 if ($(this).val() == "回退") {
				$('.accessory').hide();
		            ...
		            <div class="ibox-content">
                       <form id="editForm" action="${ctx}/jsp/work/subGovernmentResources.do"  method="post" class="form-horizontal key-13" >
						   ...
						   <div class="form-group">
								...
								<div class="col-sm-4">
									<label>
										<input type="radio" name="zdzc_result" value="完成资管工单派发" checked="checked" id="archive"/><span style="padding: 0px 10px 0px 5px;">完成工单派发</span>
									</label>
									<label>
										<input type="radio" name="zdzc_result" value="回退"/><span style="padding: 0px 10px 0px 5px;">退回</span>
									</label> 
									<label>
										<input type="radio" name="zdzc_result" value="回退到全业务支撑"/><span style="padding: 0px 10px 0px 5px;">回退到全业务支撑</span>
									</label> 
								</div>
							</div>
