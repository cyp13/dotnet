...
function $addFile(dom){
	//禁用的"+"直接返回
	if( $(dom).attr("disabled") || $(dom).attr("disabled") == "disabled" ){
		return;
	}
	...
	/*
	 * 文件域的父元素配置文件数量限制
	 * <div isLimit="1" limit="5">
			<input type="file" id="file" name="file" />
	   </div>
	 */
	var isLimit = $(dom).closest(".diyFile").parent().attr("isLimit");
	if( "1" == isLimit ){
		var limit = $(dom).closest(".diyFile").parent().attr("limit");
		var l = $(dom).parent().parent().parent().siblings().length;
		if( l == ( limit - 1 ) ){
			//文件域数量已达到限制,禁用文件添加
			$(dom).closest(".diyFile").parent().find(".fa-plus").parent().attr("disabled", true);
		}
	}
}
//删除inputfile框
function $deleteFile(dom){
	...
	if( "1" == isLimit ){
		...
		if( l < limit ){
			//文件域数量少于限制,启用文件添加
			$(dom).closest(".diyFile").parent().find(".fa-plus").parent().attr("disabled", false);
		}
	}
