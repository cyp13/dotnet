...
;(function(root, factory) {
    ...
    //选择日期后触发父页面记录方法
    jedfn.logChange = function (obj) {
        if (window.parent && window.parent.window.DatePickerChanged) {
            var text = window.parent.window.findText(obj);
            if (text != null) {
                window.parent.window.DatePickerChanged(text, $(obj).val());
            } else {
                window.parent.window.DatePickerChanged($(obj).attr('id'), $(obj).val());
            }
        }
    };
    //选择日期
