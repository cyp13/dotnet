	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		if ( /input/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
			var html = popup.formatHtml(
				'<nobr>单行文本框: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
			if ( html ) {
    ...
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                ...
                editor:this,
                title: '下拉框',
                cssRules:"width:590px;height:380px;",
                buttons:[
                {
                    className:'edui-okbutton',
                    label:'确定',
    ...
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                ...
                editor:this,
                title: '表单设计器 —控件清单',
                cssRules:"width:500px;height:123px;",
                buttons:[
                {
                    className:'edui-okbutton',
                    label:'确定',
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'org');
 * ```
 */
UE.plugins['org'] = function () {
	var me = this,thePlugins = 'org';
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/org.html',
				name:thePlugins,
				editor:this,
				title: '机构',
				cssRules:"width:600px;height:310px;",
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		if ( /input/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
			var html = popup.formatHtml(
				'<nobr>机构: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
			if ( html ) {
				popup.getDom( 'content' ).innerHTML = html;
				popup.anchorEl = el;
				popup.showAnchor( popup.anchorEl );
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'user');
 * ```
 */
UE.plugins['user'] = function () {
	var me = this,thePlugins = 'user';
	me.commands[thePlugins] = {
			execCommand:function () {
				var dialog = new UE.ui.Dialog({
					iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/user.html',
					name:thePlugins,
					editor:this,
					title: '人员',
					cssRules:"width:600px;height:310px;",
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		if ( /input/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
			var html = popup.formatHtml(
			'<nobr>人员: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
			if ( html ) {
				popup.getDom( 'content' ).innerHTML = html;
				popup.anchorEl = el;
				popup.showAnchor( popup.anchorEl );
...
UE.plugins['file'] = function () {
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		if ( /input/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
			var html = popup.formatHtml(
			'<nobr>附件: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
			if ( html ) {
				popup.getDom( 'content' ).innerHTML = html;
				popup.anchorEl = el;
				popup.showAnchor( popup.anchorEl );
