(function(b) {
	...
	a.config = {
		...
		rect : {
			attr : {
				...
				height : 50,
				r : 10,
				fill : "90-#fff-#F5F5F5", //C0C0C0 E8E8E8
				stroke : "#696969", //000
				"stroke-width" : 1
			},
			showType : "image&text",
		...
		path : {
			...
			props : {
				text : {
					...
					value : "",
					editor : function() {
						return new a.editors.textEditor("line")
					}
				}
			}
		},
	...
	a.props = function(h, f) {
		...
		var d = function(n, m, o) {
			...
			c.show();
			for ( var l in m) {
				e.append("<tr><th style='min-width:30px'>" + m[l].label + '</th><td><div id="p' + l
	...
	a.editors = {
		textEditor : function(type) {
			...
			this.init = function(i, h, m, l, j) {
				...
				g = l;
				f = j;
				var o = b('<input style="width:100%;"/>');
				if("line" == type) {
					$(o).attr("title", "必填");
					$(o).attr("placeholder", "必填");
				} else {
					$(o).attr("title", "必填(唯一)");
		...
		if (a.config.editable) {
			...
			b(".save").click(
					function() {
						...
						i += "}}}";
						a.config.tools.publish.onclick(i, $(this).attr("deploymentId"))
					});
			b("#myflow_export_").click(
					function() {
						var i = "{states:{";
						for ( var c in q) {
							if (q[c]) {
						...
						i += "}}}";
						a.config.tools.export_.onclick(i)
					});
			b("#myflow_import_").click(
					function() {
						var i = "{states:{";
						for ( var c in q) {
							if (q[c]) {
						...
						i += "}}}";
						a.config.tools.import_.onclick(i)
					});
			new a.props({}, y)
		}
		if (r.restore) {
			...
			if(B.jbpmName && B.jbpmName.props && B.jbpmName.props.name && B.jbpmName.props.name.value) { //yove add
				a.config.props.props.name.value = B.jbpmName.props.name.value;
			}
			if(B.jbpmName && B.jbpmName.props && B.jbpmName.props.description && B.jbpmName.props.description.value) { //yove add
				a.config.props.props.description.value = B.jbpmName.props.description.value;
			}
		}
