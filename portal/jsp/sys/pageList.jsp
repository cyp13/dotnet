...
<script>
    var insertUrl = "${ctx}/jsp/sys/insertPage.do";
	var updateUrl = "${ctx}/jsp/sys/updatePage.do";
	var deleteUrl = "${ctx}/jsp/sys/deletePage.do";
	var queryUrl = "${ctx}/jsp/sys/queryPage.do";
	...
	$(window).ready(function() {
		...
		$("#btn-update").on("click", function() {
			...
			ajax(obj, function(result) {
				if ("error" == result.resCode) {
					...
					queryRole(result.datas[0].extId);
				}
			});
       	});
		...
		$("#menuName").on("click", function() {
			var extId = $("#extId").val();
			if(!extId) {
				layer.msg("请先选择角色！", {icon: 0});
				return;
			}
			showMenuTree(this, "2", extId, function(treeId, treeNode) {
				var nodes = menuDivTree.getCheckedNodes(true);
				if(nodes.length > 0) {
					var node = nodes[0];
		...
		ajax(obj, function(result) {
			if ("error" == result.resCode) {
				...
				$.each(result.datas, function(i,o) {
					$("#extId").append("<option value='"+o.id+"'>"+o.roleName+"</option>");    
				});
				$("#extId").selectpicker({
					style: "btn-white"
				});
				$("#extId").selectpicker("render");
				$("#extId").selectpicker("refresh");
				
				if(extId) {
					$("#extId").selectpicker("val", extId);
				}
			}
		});
	}
            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
   					<div class="ibox-title">
	                    <h5><i class="fa fa-star-o"></i> 首页列表</h5>
	        ...
	        <div class="modal-content ibox">
                <form id="editForm" class="form-horizontal key-13" role="form">
		            ...
		            <div class="ibox-content">
	                  ...
	                  <input type="text" id="pageType" name="pageType" value="2" style="display:none"/>
	                  <input type="text" id="rowVersion" name="rowVersion" style="display:none"/>
	                  
					  <div class="form-group">
					    <label class="col-sm-2 control-label">角色名称</label>
					    <div class="col-sm-4">
					      <select id="extId" name="extId" class="form-control validate[required]" data-live-search="true" ></select>
					    </div>
					    <label class="col-sm-2 control-label">菜单名称</label>
					    <div class="col-sm-4">
					      <input type="text" class="input-sm form-control validate[required]" name="menuName" id="menuName" readonly="readonly" placeholder="请选择">
